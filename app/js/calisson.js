var calisson = function() {
  ...
  var gridConfig = { 
   ...
   canvas.add(grid(gridConfig));
    ...
    for (var row = 0; row < gridConfig.radius+1; row++) {
      for (var col = -row/2+0.5; col < row/2; col++) {
        canvas.add(ca().set(adjustOffset({top: row/2 - .5, left: col*2}, gridConfig)));
      }
    }
    for (var row = 0; row < gridConfig.radius; row++) {
      for (var col = -row/2+0.5; col < row/2; col++) {
        canvas.add(ca().set(adjustOffset({top: gridConfig.radius-row/2 - .5, left: col*2}, gridConfig)));
      }
    }

    console.log(canvas._objects);
    canvas.renderAll();
    // console.log(JSON.stringify(canvas.item(0).item(0)).replace(/,/g, ',\n'));
   // alert(JSON.stringify(canvas.item(0).item(0)).replace(/,/g, ',\n'));
  }

  function main() {
    var timer = Math.max(10, $('input#reload-every').val());
    console.log('Reloading in ' + timer + ' seconds.');
    setTimeout(maybeReload, timer * 1000);
    try {
